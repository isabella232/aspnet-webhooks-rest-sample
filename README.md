...
| The client does not receive notifications. | If you're using ngrok, you can use the web interface (*http://127.0.0.1:4040*) to see whether the notification is being sent. If you're not using ngrok, monitor the network traffic using the tools your hosting service provides, or try using ngrok.<br />If Microsoft Graph is not sending notifications, please open a [Stack Overflow](https://stackoverflow.com/questions/tagged/MicrosoftGraph) issue tagged with *[MicrosoftGraph]*. Include the subcription ID, the time it was created, and the correlation ID from the response (if you have it). |
